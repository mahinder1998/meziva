// ../../node_modules/.pnpm/tsup@8.5.1_jiti@2.4.2_postcss@8.5.6_typescript@4.9.5_yaml@2.8.2/node_modules/tsup/assets/esm_shims.js
      ...
      const generatedConfig = {
        ...
        server: {
          ...
          allowedHosts: config.server?.allowedHosts ?? [
            ...
            })() : options.tunnel ? [".trycloudflare.com"] : []
          ],
          cors: config.server?.cors ?? {
            origin: config.server?.origin ?? [
              /^https?:\/\/(?:(?:[^:]+\.)?localhost|127\.0\.0\.1|\[::1\])(?::\d+)?$/,
              // allows localhost (default)
              /\.myshopify\.com$/
              // allows myshopify.com URLs
            ]
          }
        }
      };
...
var hotReloadScriptId = "hot-reload-client";
var hotReloadScriptUrl = "https://cdn.jsdelivr.net/npm/@shopify/theme-hot-reload/dist/theme-hot-reload.min.js";
    ...
    configureServer({ config: config2, middlewares, httpServer }) {
      ...
      httpServer?.once("listening", () => {
        ...
        if (isAddressInfo(address)) {
          ...
          debug2({ address, viteDevServerUrl, frontendUrl, frontendPort, usingLocalhost });
          setTimeout(() => {
            void (async () => {
...
var viteTagEntryPath = (resolveAlias, entrypointsDir, snippetName) => {
  ...
  const replaceChain = replacements.map(([from, to]) => `replace: '${from}/', '${to}/'`).join(" | ");
  return `{% liquid
...
var viteTagSnippetDev = (assetHost, entrypointsDir, reactPlugin, themeHotReload) => `{% liquid
...
function generateFrontendURL(options) {
...
// src/liquid-no-refresh.ts
import createDebugger3 from "debug";
var debug3 = createDebugger3("vite-plugin-shopify:liquid-no-refresh");
function shopifyLiquidNoRefresh() {
  return {
